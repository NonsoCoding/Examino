...
export function JambQuestionListBiology({navigation}) {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      while (fetchedQuestions.length < 5) {
        const response = await fetch(`https://questions.aloc.com.ng/api/v2/q?subject=biology&type=utme&page=${currentPage}`, {
          headers: {            
  ...
  const gradeUser = () => {
    ...
    alert(`you got ${correctAnswers} out of ${questions.length} questions right. your percentage is ${percentage.toFixed(2)}%.`)
  }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Theme.colors.primary }}>
      <TouchableOpacity style={{paddingLeft: 20, flexDirection: "row", alignItems: "center"}} onPress={() => navigation.navigate("BiologyExamInfoScreenJamb")}>
      ...
      <View style={styles.container}>
        {loading ? (
          ...
          <View style={styles.gradeContainer}>
            <View style={{borderRadius: 125, padding: 10, borderWidth: 1, height: 250, width: 250, alignItems: "center",justifyContent: "center", marginBottom: 20, borderColor: "white"}}>
            <Text style={[styles.gradeText, {color: "white"}]}>Your percentage is </Text>
            <Text style={{fontFamily: Theme.fonts.text900, fontSize: 19, color: "white"}}>{((correctAnswers / questions.length) * 100).toFixed(2)}%</Text>
            </View>
            <Text style={styles.gradeText}>Congratulations you got {correctAnswers} out of {questions.length} questions right.</Text>
            <TouchableOpacity style={styles.gradeButton} onPress={fetchQuestions}>
          ...
          <>
            <View style={styles.questionContainer}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Text style={{fontFamily: Theme.fonts.text600}}>Exam type: {questions[currentQuestionIndex].examtype}</Text>
                <Text>,  Exam Year: {questions[currentQuestionIndex].examyear}</Text>
              </View>
            </View>
  ...
  {["a", "b", "c", "d", "e"].map((optionKey) => (
    ...
    >
      <Text style={styles.optionText}>
        {`${optionKey.toUpperCase()}. ${questions[currentQuestionIndex].option[optionKey]} `}
        {selectedOption === optionKey && (
          <FontAwesome5 name="check-circle" size={20} color="green" style={{ marginLeft: 5 }} />
        )}
      </Text>
    </TouchableOpacity>
  ))}

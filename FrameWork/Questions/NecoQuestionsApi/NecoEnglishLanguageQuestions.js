...
export function NecoQuestionListEnglishLanguage({ navigation }) {
    ...
    try {
      ...
      while (fetchedQuestions.length < 5) {
        const response = await fetch(
          `https://questions.aloc.com.ng/api/v2/q?subject=english&type=neco&page=${currentPage}`,
          {
            headers: {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Theme.colors.primary }}>
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate("NecoEnglishLanguageExamInfoScreen")}
      >

...
export function QuestionListMathematics({ navigation }) {
    ...
    try {
      ...
      while (fetchedQuestions.length < 5) {
        const response = await fetch(
          `https://questions.aloc.com.ng/api/v2/q?subject=mathematics&type=waec&page=${currentPage}`,
          {
            headers: {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Theme.colors.primary }}>
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate("MathematicsExamInfoScreen")}
      >
